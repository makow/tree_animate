...
void two_four_insert_balance (Tree *T, Node *x)
{
    Node *p_sibling, *g; p_sibling = g = NULL;
    bool side, p_side; side = p_side = 0;

    while (x->rd == 0 && x->p->rd == 0 &&
           x->p != T->head && x->p->p != T->head) {             // VIOLATION
        g = x->p->p;
        p_sibling = return_sibling (x->p);
        if (p_sibling->rd == 0) {                               // Case 1
            promote (T, g);
            x = g;
        }
        else if (p_sibling->rd == 1) {
            side = (x == x->p->child[R]);
...
void two_four_delete_balance (Tree *T, Node *x)
{
    Node *sibling = NULL;
    bool side = 0;
